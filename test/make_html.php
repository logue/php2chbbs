<?php
define('VERSION', '2015/01/31');
# 過去ログメニューをかーくー
$fp = @fopen($SUBFILE, "w");
if (!$fp) exit;
fputs($fp, '<!DOCTYPE html><html><head><meta charset="UTF-8" /><meta http-equiv="pragma" content="no-cache" /><meta http-equiv="Cache-Control" content="no-cache" /></head><body><font size="2">');
$count = 0;
...
#--------ヘッダ＆上の広告
list($header, $footer) = explode('<CUT>', join('', file("../test/index.txt")));
...
#--------スレッド表示
$i = 1;
$form_txt = join('', file("../test/form.txt"));
...
$end = count($th_titles);
$data = '<!DOCTYPE html><html><head><meta charset="UTF-8" /><title>' . $_REQUEST['bbs'] . ' スレッド一覧</title></head><body>' . $SETTING['BBS_TITLE'] . '<hr />';
for ($i = 1; $i < 11; $i++) {
...
strstr($_SERVER['HTTP_USER_AGENT'], 'UP.Browser')) {
	header('Location: //'.$_SERVER['HTTP_HOST'].dirname(dirname($_SERVER['SCRIPT_NAME'])).'/' . $_REQUEST['bbs'] . '/i/');
	exit;
}
...
<title>書きこみました。</title>
<?php echo $set_cookie?>
<meta http-equiv="refresh" content="1;URL=<?php echo $INDEXFILE; ?>?" />
</head>
<body>書きこみが終わりました。<br />
<br />
